        ...
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div class="addr-info" v-bind:class="{'checked':index == checkIndex}" @click="checkIndex=index" v-for="(item,index) in list" :key="index">
            ...
            <ul>
              <li v-for="(item,index) in cartList" :key="index">
                <div class="good-name">
                  ...
                  <span>{{item.productName + ' ' + item.productsSubtitle}}</span>
                </div>
...
export default{
  ...
  data(){
    return {
      ...
      showDelModal:false,//是否显示删除弹框,默认是false（不弹出）
      showEditModal:false,//是否显示新增或者编辑弹框，默认是false（不弹出）
  ...
  methods:{
    ...
    //打开新增地址弹框
    openAddressModal(){
      this.userAction = 0;//该句话含义是，用户的行为是0，代表新增
      this.checkedItem = {};
      this.showEditModal = true;
    },
    //编辑地址弹框
    editAddressModal(item){
      this.userAction = 1;//该句话含义是，用户的行为是1，代表编辑
      this.checkedItem = item;
      this.showEditModal = true;
    },
    delAddress(item){
      this.checkedItem = item;//该句话含义是，将删除的地址保存下来
      this.userAction = 2;//该句话含义是，用户的行为是2，代表删除
      this.showDelModal = true;//将弹框设置为true，显示出来
    },
    //地址删除、编辑、新增功能
    submitAddress(){
      let {checkedItem,userAction} = this;
      //method变量代表发送请求的方法，url变量代表发送请求的地址
    ...
    getCartList(){
        ...
        //filter()是一个带有过滤功能的方法
        this.cartList = list.filter(item=>item.productSelected);//过滤出购物车中商品数据，所有选中的商品，赋值给cartList（购物车中需要结算的商品列表）
